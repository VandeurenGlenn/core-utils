...
import { gnuwin32Version, grepUrl, patchUrl, url } from './constants.js';
...
export const downloadCommands = async () => {
  ...
  await download(patchUrl, './.cache') 
  await download(grepUrl, './.cache') 
}

export const extract = async (dest, options) => 
  Promise.all([
    decompress(join(CACHE_PATH, 'coreutils-5.3.0-bin.zip'), dest, options),
    decompress(join(CACHE_PATH, 'grep-2.5.4-bin.zip'), dest, options),
    decompress(join(CACHE_PATH, 'patch-2.5.9-7-bin.zip'), dest, options)
  ])
